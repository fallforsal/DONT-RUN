...
public class AuthManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (!string.IsNullOrEmpty(savedToken))
        {
            ...
            // KIỂM TRA PHÉP MÀU: Nếu mạng vẫn đang kết nối (Tức là Server vừa Reset Map xong)
            if (Mirror.NetworkClient.active)
            {
                Debug.Log("[Auth] Server vừa reset map! Đã kết nối sẵn, bỏ qua Sảnh chờ...");
                if (lobbyPanel != null) lobbyPanel.SetActive(false); // Bắt buộc tắt Lobby để giấu nút Join đi
            }
            else
            {
                Debug.Log("[Auth] Đã tìm thấy Token, tự động vào Sảnh!");
                if (lobbyPanel != null) lobbyPanel.SetActive(true);
            }
        }
        else
        {
            // Lần đầu mở game chưa có Token
            SwitchToLoginState();
    ...
    {
        // 1. Xóa sạch chìa khóa trong bộ nhớ
        PlayerPrefs.DeleteKey("JWT_Token");
        PlayerPrefs.DeleteKey("CurrentPlayerName");
        PlayerPrefs.Save();

        // 2. Tắt Sảnh, văng ra màn hình Đăng nhập
        if (lobbyPanel != null) lobbyPanel.SetActive(false);
        loginRegisterPanel.SetActive(true);
        SwitchToLoginState();
        
        SetMessage("Logged out successfully!", Color.green);
